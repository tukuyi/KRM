	...
	[[		
		float4 main( VS_OUTPUT_TERRAIN Input ) : PDX_COLOR
		{
			...
			if (rlDelta.r < 0.02 && rlDelta.g < 0.02 && rlDelta.b < 0.04 ) {
				...
				// 椭圆距离（x轴补偿地图比例）
				float rlDist = length(float2(2.0 * (Input.uv.x - rlCenter.x), Input.uv.y - rlCenter.y));

				// 碎片化噪声：用多个sin叠加模拟不规则破碎感
				float rlNoise = sin(Input.uv.x * 173.7 + vGlobalTime * 1.3)
							* sin(Input.uv.y * 211.3 - vGlobalTime * 0.9)
							* sin((Input.uv.x + Input.uv.y) * 97.1 + vGlobalTime * 2.1);
				rlNoise *= 0.018; // 控制碎片扰动幅度

				// 扩散波角，加噪声碎片化
				float rlWaveAngle = (rlDist + rlNoise) * 35.0 - vGlobalTime * 0.7;

				// 波形：sin²产生圆环，乘以额外sin制造不均匀脉冲
				float rlWave = sin(rlWaveAngle) * sin(rlWaveAngle);
				rlWave *= 0.5 + 0.5 * sin(rlWaveAngle * 0.5 + vGlobalTime * 1.7); // 脉冲不均匀
				rlWave *= 0.15;

				// 颜色选择：以绿色为主的诡异彩虹
				float rlColorSel = frac(rlWaveAngle / 3.141592);

				float3 rlC1 = float3(0.0,  0.9,  0.1);  // 毒绿
				float3 rlC2 = float3(0.1,  0.6,  0.0);  // 暗绿
				float3 rlC3 = float3(0.4,  1.0,  0.2);  // 亮绿
				float3 rlC4 = float3(0.0,  0.5,  0.3);  // 青绿
				float3 rlC5 = float3(0.5,  0.0,  0.8);  // 诡异紫（偶发）
				float3 rlC6 = float3(0.8,  0.1,  0.0);  // 血红（偶发）

				// 绿色占4/6段，异色仅2/6段
				float3 rlColorStart = rlC1;
				float3 rlColorEnd   = rlC2;
				if      (rlColorSel < 1.0/6.0) { rlColorStart = rlC1; rlColorEnd = rlC2; }
				else if (rlColorSel < 2.0/6.0) { rlColorStart = rlC2; rlColorEnd = rlC3; }
				else if (rlColorSel < 3.0/6.0) { rlColorStart = rlC3; rlColorEnd = rlC4; }
				else if (rlColorSel < 4.0/6.0) { rlColorStart = rlC4; rlColorEnd = rlC1; }
				else if (rlColorSel < 5.0/6.0) { rlColorStart = rlC1; rlColorEnd = rlC5; } // 绿->紫
				else                            { rlColorStart = rlC5; rlColorEnd = rlC6; } // 紫->血红

				float3 rlShieldColor = lerp(rlColorStart, rlColorEnd, frac(6.0 * rlColorSel));

				vOut.rgb = vOut.rgb * (1.0 - rlWave) + rlShieldColor * rlWave;
			}
